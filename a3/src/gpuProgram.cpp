...
void GPUProgram::validateShader( GLuint shader, const char* file, const char *shaderName, const char *shaderType )

{
  ...
  if (length > 0) {
    std::cout << "Shader " << shader << " (" << shaderName << " " << shaderType << ") compile log: " << std::endl << buffer << std::endl;
    exit(1);
  }
}
...

{
  ...
  validateShader( shader_vp, "vertex shader", shaderName, "vertex shader" );
  ...
  validateShader( shader_fp, "fragment shader", shaderName, "fragment shader" );
